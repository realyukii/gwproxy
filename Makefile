...
LIBGWDNS_CC_SOURCES = $(GWPROXY_DIR)/dns.c
LIBGWDNS_OBJECTS = $(LIBGWDNS_CC_SOURCES:%.c=%.c.o)

ALL_OBJECTS = $(GWPROXY_OBJECTS) $(LIBGWPSOCKS5_OBJECTS) $(LIBGWDNS_OBJECTS)
ALL_TARGETS = $(GWPROXY_TARGET) $(LIBGWPSOCKS5_TARGET) $(LIBGWDNS_TARGET)
ALL_DEPFILES = $(ALL_OBJECTS:.o=.o.d)

all: $(GWPROXY_TARGET) $(LIBGWPSOCKS5_TARGET) $(LIBGWDNS_TARGET)

$(GWPROXY_TARGET): $(ALL_OBJECTS)
	$(CC) $(LDFLAGS) -o $@ $^ $(LIBS)
